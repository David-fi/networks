...
public class TemporaryNode implements TemporaryNodeInterface {
    ...
    public String get(String key) {
        ...
        try {
            int keyLines = key.endsWith("\n") ? key.split("\n", -1).length - 1 : key.split("\n", -1).length;
            ...
            if (response != null && response.startsWith("VALUE")) {
                ...
                for (int i = 0; i < valueLines; i++) {
                    ...
                    System.out.println("Line " + i + ": " + line); // Debugging
                }
                return valueBuilder.toString();
            } else {
                System.out.println("No VALUE line or response was: " + response); // Debugging
                return null;
            }
        } catch (IOException e) {
